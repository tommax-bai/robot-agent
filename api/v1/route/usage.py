...
async def get_daily_stats(date: str = Query(None, description="查询日期，格式 YYYY-MM-DD，不传默认今天")):
    """
    ...
    log_file = f"{LOG_DIR}/usage_{date}.jsonl"
...
async def get_usage_details(
    date: str = Query(None, description="查询日期，格式 YYYY-MM-DD，不传默认今天"),
...
async def list_available_dates():
    ...
    dates = [os.path.basename(f).replace("usage_", "").replace(".jsonl", "") for f in files]
