...
def open_rednote_homepage(trace_id: str):
    """
    打开小红书首页。
    策略：
    1. 检查是否已有小红书标签页，有则切换。
    ...
    # 1. 寻找现有标签页
    for tab in tabs:
        if "小红书" in tab["title"] or "xiaohongshu.com" in tab["url"]:
            logger.info({"msg": "发现已有小红书标签页，正在切换", "tab_id": tab["id"]}, trace_id)
            chrome_client.switch_to_tab(tab["id"])
            return {"ok": True, "message": "已切换到现有的小红书标签页", "finish": False}

    # 2. 如果没有，通过 API 打开新标签页
    target_url = "https://www.xiaohongshu.com/explore"
    logger.info({"msg": "未发现小红书标签页，通过 API 打开新页面", "url": target_url}, trace_id)
    ...
    return {"ok": True, "message": "已成功打开并切换到小红书首页", "finish": False}
