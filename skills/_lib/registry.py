...
class SkillRegistry:
    ...
    def prompt_section(self, pack_names: Iterable[str]) -> str:
        """拼装挂载技能的 prompt 段落。空列表返回空串。"""
        parts: list[str] = []
        ...
        if not parts:
            return ""
        return "\n## 挂载技能知识库 (Expert Skills)" + "".join(parts)
    ...
    def invoke(self, name: str, params: dict, trace_id: str = "system") -> dict:
    ...
    def build_prompt(self, skill_names: list[str]) -> str:
        return self.prompt_section(skill_names)

    def invoke_tool(self, tool_name: str, params: dict, trace_id: str = "system") -> dict:
        return self.invoke(tool_name, params, trace_id)
