...
class ChromeClient:
    ...
    def list_tabs(self) -> List[Dict[str, Any]]:
        ...
        try:
            resp = requests.get(
                f"http://{config.settings.system.chrome.chrome_ip}:{config.settings.system.chrome.debug_port}/json", timeout=2
            )
    ...
    def switch_to_tab(self, tab_id: str) -> bool:
        ...
        try:
            # 通过 CDP 激活标签页
            resp = requests.get(
                f"http://{config.settings.system.chrome.chrome_ip}:{config.settings.system.chrome.debug_port}/json/activate/{tab_id}",
                timeout=2,
            )
            if resp.status_code == 200:
                # 重新连接到新标签页
    ...
    def new_tab(self, url: str = "about:blank") -> str | None:
        ...
        try:
            resp = requests.put(
                f"http://{config.settings.system.chrome.chrome_ip}:{config.settings.system.chrome.debug_port}/json/new?{url}",
                timeout=5,
            )
            if resp.status_code == 200:
    ...
    def close_tab(self, tab_id: str) -> bool:
        ...
        try:
            resp = requests.get(
                f"http://{config.settings.system.chrome.chrome_ip}:{config.settings.system.chrome.debug_port}/json/close/{tab_id}",
                timeout=2,
            )
    ...
    def _start_chrome(self) -> bool:
        ...
        try:
            ...
            os.makedirs(chrome.profile_dir, exist_ok=True)
            cmd = chrome.chrome_command
            ...
            if config.settings.system.system_info == "win32":
        ...
        for i in range(max_retries):
            try:
                ...
                options.add_experimental_option(
                    "debuggerAddress", f"{chrome.chrome_ip}:{chrome.debug_port}"
                )
                ...
                if sys_.system_info == "win32":
                    ...
                    service = webdriver.ChromeService(
                        executable_path=chrome.chromedriver_path, log_output=os.devnull
                    )
...
def get() -> ChromeClient:
    """获取单例。"""
    if _instance is None:
        raise RuntimeError("请先调用 bootstrap.chrome_client.start()")
    return _instance
...
def disable_auto_restart() -> None:
    """关闭流程前调用：让监控线程不再"自动重启被 SIGINT 干掉的 Chrome"。

    实际 quit driver 由 `close()` 完成；本函数只翻一个 flag。
    """
    if _instance is not None:
